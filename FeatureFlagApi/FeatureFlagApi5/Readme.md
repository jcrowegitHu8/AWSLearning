...
WORKDIR /src
COPY ["FeatureFlagApi5.csproj", "FeatureFlagApi5/"]
RUN dotnet restore "FeatureFlagApi5/FeatureFlagApi5.csproj"

WORKDIR "/src/FeatureFlagApi5"
COPY . .
RUN dotnet build "FeatureFlagApi5.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "FeatureFlagApi5.csproj" \
...
Execute unit tests
```
    cd "FeatureFlagApi5/test/FeatureFlagApi5.Tests"
    dotnet test
```

Deploy application
```
    cd "FeatureFlagApi5/src/FeatureFlagApi5"
